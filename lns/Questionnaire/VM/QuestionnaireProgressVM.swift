...
class QuestionnaireProgressVM: UIView {
    
    let selfHeight = kFitWidth(44)
    var progressWidth = SCREEN_WIDHT-kFitWidth(44)-kFitWidth(80)
    var progressWidthPerStep = kFitWidth(25.5)
    ...
    override init(frame:CGRect){
        ...
        progressWidthPerStep = progressWidth/CGFloat(totalStep)
        initUI()
    }
...
//        progressWidthPerStep = kFitWidth(255)/CGFloat(self.totalStep)
        ...
        UIView.animate(withDuration: 0.45,delay: 0,options: .curveEaseInOut) {
            self.progressView.snp.remakeConstraints { make in
                ...
                make.width.equalTo(self.progressWidthPerStep*CGFloat(step-self.startStep))
            }
            self.layoutIfNeeded()
        }
