...
@interface ForumListVideoControlView () <ZFSliderViewDelegate>
/// 竖屏控制层的View
...
@property (nonatomic, strong) UIView *muteTapView;

@end

@implementation ForumListVideoControlView
...
//        [self addGestureRecognizer:longPress];
        ...
        UITapGestureRecognizer *mutedTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(mutedTapAction)];
        [self.muteTapView addGestureRecognizer:mutedTap];
    }
    return self;
}

-(void)mutedTapAction{
    if (self.mutedCallback) {
        self.mutedCallback();
    }
}
...
//    self.rateView.zf_centerY = min_view_h - 80;
    if (self.player.isFullScreen){
        if (bottomSafeAreaHeight > 0){
            self.rateView.zf_centerX = min_view_w - 70 - bottomSafeAreaHeight - 44;
        }else{
            self.rateView.zf_centerX = min_view_w - 70;
        }
        
        self.rateView.zf_centerY = min_view_h - 120 - bottomSafeAreaHeight;//min_view_h - 80;
    }else{
    ...
    min_w = 18;
    min_h = 18;
    self.mutedImgView.frame = CGRectMake(min_view_w-8-min_w, min_view_h-8-min_h, min_w, min_h);
    self.muteTapView.frame = CGRectMake(0, 0, min_w*3, min_h*3);
    self.muteTapView.zf_centerX = self.mutedImgView.zf_centerX;
    self.muteTapView.zf_centerY = self.mutedImgView.zf_centerY;
}
...
//    [self addSubview:self.activity];
    ...
    [self addSubview:self.mutedImgView];
    [self addSubview:self.muteTapView];
}
...
- (void)resetControlView {
    self.portraitControlView.hidden = true;
    self.landScapeControlView.hidden = true;
...
- (UIImageView *)mutedImgView{
    if (!_mutedImgView) {
        _mutedImgView = [[UIImageView alloc]init];
        _mutedImgView.image = [UIImage imageNamed:@"forum_player_mute_yes_icon"];//forum_player_mute_no_icon
        _mutedImgView.contentMode = UIViewContentModeScaleAspectFit;
        [_mutedImgView setUserInteractionEnabled:YES];
    }
    return _mutedImgView;
}

- (UIView *)muteTapView{
    if (!_muteTapView) {
        _muteTapView = [[UIView alloc]init];
        [_muteTapView setUserInteractionEnabled:YES];
        _muteTapView.backgroundColor = [UIColor clearColor];
    }
    return _muteTapView;
}
