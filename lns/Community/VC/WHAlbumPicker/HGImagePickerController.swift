...
class HGImagePickerController: UIViewController {
    ...
    var videoItems:[HGImageAlbumItem] = []
    ...
    override func awakeFromNib() {
        ...
        PHPhotoLibrary.requestAuthorization({ (status) in
            ...
            self.convertCollectionVideo(collection: smartAlbums)
            
            //列出所有用户创建的相册
            let userCollections = PHCollectionList.fetchTopLevelUserCollections(with: nil)
            self.convertCollection(collection: userCollections
                as! PHFetchResult<PHAssetCollection>)
            self.convertCollectionVideo(collection: userCollections
                as! PHFetchResult<PHAssetCollection>)
            ...
            DispatchQueue.main.async{
                ...
                if let imageCollectionVC = self.storyboard?
                    ...
                    imageCollectionVC.selectStatusArray = [Int]()
                    imageCollectionVC.dataSourceArray.removeAll()
                    for i in 0..<(self.items.first?.fetchResult.count ?? 0){
                        imageCollectionVC.selectStatusArray.append(0)
                        imageCollectionVC.dataSourceArray.append(AssetModel(isSelect: false,selectIndex: 0))
                    }
    ...
    private func convertCollection(collection:PHFetchResult<PHAssetCollection>){
        for i in 0..<collection.count{
            ...
            //如果已经选择过照片，再次进相册的时候，不再显示视频
            if UserConfigModel.shared.selectType == .IMAGE && UserConfigModel.shared.photsSelectCount > 0{
                resultsOptions.predicate = NSPredicate(format: "mediaType = %d || mediaType = %d",
...
//            let resultVidewsOptions = PHFetchOptions()
//            resultVidewsOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate",
//                                                               ascending: false)]
//            resultVidewsOptions.predicate = NSPredicate(format: "mediaType = %d",
//                        PHAssetMediaType.video.rawValue)
            let c = collection[i]
    ...
    //转化处理获取到的相簿
    private func convertCollectionVideo(collection:PHFetchResult<PHAssetCollection>){
        for i in 0..<collection.count{
            //获取出但前相簿内的图片
            let resultsOptions = PHFetchOptions()
            resultsOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate",
            ...
            if assetsFetchResult.count > 0 {
                let title = titleOfAlbumForChinse(title: c.localizedTitle)
                videoItems.append(HGImageAlbumItem(title: title,
                                              fetchResult: assetsFetchResult))
            }
        }
    }
...
//            imageCollectionVC.title = cell.titleLabel.text
            imageCollectionVC.navigationController?.title = cell.titleLabel.text
            //设置最多可选图片数量
            imageCollectionVC.maxSelected = self.maxSelected
            guard  let indexPath = self.tableView.indexPath(for: cell) else { return }
            imageCollectionVC.selectStatusArray = [Int]()
            imageCollectionVC.dataSourceArray.removeAll()
            for i in 0..<(self.items[indexPath.row].fetchResult.count ?? 0){
                imageCollectionVC.selectStatusArray.append(0)
                imageCollectionVC.dataSourceArray.append(AssetModel(isSelect: false,selectIndex: 0))
            }
            //获取选中的相簿信息
