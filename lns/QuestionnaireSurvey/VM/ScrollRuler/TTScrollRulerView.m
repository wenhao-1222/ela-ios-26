...
- (void)initialization {
    ...
    _pointerFrame = CGRectMake(cy_selfWidth/2.0-cy_fit(1)/2.0, cy_selfHeight/2.0-(cy_selfHeight>=cy_fit(120)?cy_fit(60):cy_selfHeight/2.0), cy_fit(2), cy_selfHeight>=cy_fit(120)?cy_fit(60):cy_selfHeight/2.0);
...
- (void)addView {
    ...
    _rulerBackgroundView.decelerationRate = 0.5;
...
- (void)rulerInit {
    ...
    if (_rulerDirection == RulerDirectionHorizontal) {
        ...
        if (!_pointerFrameSeted && _rulerFace == RulerFace_up_left) {
            _pointerView.frame = CGRectMake(cy_selfWidth/2.0-(cy_selfHeight>=cy_fit(120)?cy_fit(80):cy_selfHeight/2.0), cy_selfHeight/2.0-cy_fit(1)/2.0, cy_selfWidth>=cy_fit(120)?cy_fit(80):cy_selfWidth/2.0, cy_fit(44));
...
//        [self.rulerDelegate rulerWith:self.i*_unitValue];
        NSInteger valueGap = self.i - _lockMin;
        NSInteger gap = _lockMax-valueGap;
        [self.rulerDelegate rulerWith:gap*_unitValue];
    }else {
        NSAssert(NO, @"error");
    }
}
