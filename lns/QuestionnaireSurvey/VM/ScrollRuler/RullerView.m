...
- (void)drawRect:(CGRect)rect {
    ...
    NSInteger maxValue = _lockMax/_unitValue;
    NSInteger minValue = _lockMin/_unitValue;
    //画刻度
    for (NSInteger i=_lockMax/_unitValue; i>(_lockMin/_unitValue-1); i--) {
...
//            aPoints[1] =CGPointMake(_mark_bottom,_pointerFrame.origin.y+_pointerFrame.size.height/2.0+_unitPX*i);//终点
        ...
        if (i%10==0){
            CGContextSetLineWidth(context, cy_fit(2));
        }else{
            CGContextSetLineWidth(context, cy_fit(1));
        }
        CGContextAddLines(context, aPoints, 2);//添加线
        CGContextDrawPath(context, kCGPathStroke); //根据坐标绘制路径
        ...
        if (_isShowRulerValue && i%10==0) {
            ...
            if (_rulerDirection == RulerDirectionHorizontal) {
                ...
                //NSInteger i=_lockMax/_unitValue; i>(_lockMin/_unitValue+1); i--
                //NSInteger i=_lockMin/_unitValue; i<(_lockMax/_unitValue+1); i++
                
                NSInteger valueGap = maxValue - i;
                NSInteger gap = minValue + valueGap;
                
                [@(i*_unitValue).stringValue drawInRect:CGRectMake(_num_top, _pointerFrame.origin.y+_pointerFrame.size.height/2.0+_unitPX*(gap-1), _unitPX*8, _num_height) withAttributes:attributes];
//                NSLog(@"%ld --- %.2f",(long)i,(_pointerFrame.origin.y+_pointerFrame.size.height/2.0+_unitPX*(i-1)));
            }else {
                NSAssert(NO, @"error");
            }
