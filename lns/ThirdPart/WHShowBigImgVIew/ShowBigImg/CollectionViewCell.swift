...
//        scroll.bounces = false
    ...
    var imgUrl: String? {
        didSet {
            ...
            var signUrl = img_url
            DSImageUploader().dealImgUrlSignForOss(urlStr: img_url) { signStr in
                signUrl = signStr
                self.imgView.sd_setImage(with: URL(string: signUrl), placeholderImage: UIImage.imageWithColor(color: UIColor.init(red: 0, green: 0, blue: 0, alpha: 1)), options: .allowInvalidSSLCertificates) { [weak self](img, error, _, url) in
