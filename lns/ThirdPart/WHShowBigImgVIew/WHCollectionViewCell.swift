...
class WHCollectionViewCell: UICollectionViewCell {
    
    lazy var backScroll : UIScrollView = {
        let scroll = UIScrollView.init(frame: CGRect(x: 0,y: 0, width: ScreenW, height: ScreenH))
//        let scroll = UIScrollView.init(frame: CGRect(x: 0,y: 0, width: ScreenW, height: ScreenH-(UIApplication.shared.statusBarFrame.height + 44)))
    ...
    var imgUrl: String? {
        didSet {
            ...
            var imgUrlShow = img_url
            let gcdGroup = DispatchGroup()
//            if img_url.contains("ela-test-private") || img_url.contains("ela-prod-private"){
                gcdGroup.enter()
                DSImageUploader().dealImgUrlSignForOss(urlStr: img_url, completion: { str in
                    imgUrlShow = str
                    gcdGroup.leave()
                })
//            }
            gcdGroup.notify(queue: .main) {
                self.imgView.sd_setImage(with: URL(string: imgUrlShow), placeholderImage: UIImage.imageWithColor(color: UIColor.init(red: 0, green: 0, blue: 0, alpha: 1)), options: .allowInvalidSSLCertificates) { [weak self](img, error, _, url) in
                    ...
                    if h > ScreenH { //-(UIApplication.shared.statusBarFrame.height + 44) {
    //                    let y = (h - ScreenH) / 2
                        ...
                        let y = (ScreenH - h) / 2
    //                    let y = (ScreenH-(UIApplication.shared.statusBarFrame.height + 44) - h) / 2
                        DispatchQueue.main.async {
...
extension WHCollectionViewCell : UIScrollViewDelegate {
...
// MARK: 点击的方法
extension WHCollectionViewCell {
...
// MARK: 添加手势
extension WHCollectionViewCell: UIGestureRecognizerDelegate {
    // 点击手势
