...
extension HeroBrowserBaseImageCell: UIScrollViewDelegate {
    @objc private func onPan(gest: UIPanGestureRecognizer) {
        ...
        case .ended,.cancelled:
            ...
            let isRight: Bool = (gest.velocity(in: self).x > 0 && self.scrollView.contentOffset.x == 0 && UserConfigModel.shared.browerIndex == 0) ? true : false//增加右滑关闭图片
...
//        DLLog(message: "getScaleForPan:\(translation)")
        
//       let scale = min(1.0, max(0.3, 1 - translation.y / self.bounds.size.height))
        //增加右滑关闭图片
        let value = max(translation.x / self.bounds.size.width, translation.y / self.bounds.size.height)
        var scale: CGFloat = min(1.0, max(0.3, 1-value))
        return scale;
    }
...
extension HeroBrowserBaseImageCell:UIGestureRecognizerDelegate {
    ...
    open override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        if let pan = gestureRecognizer as? UIPanGestureRecognizer {
            if self.scrollView.contentOffset.y > 0 && self.scrollView.contentOffset.x != 0{
                return false
            }
            ...
            DLLog(message: "gestureRecognizerShouldBegin: \(velocity.x)  --  \(velocity.y)")
            
            if self.scrollView.contentOffset.x == 0 && velocity.x > 0 && UserConfigModel.shared.browerIndex == 0{
                return true
            }
            if velocity.y < 0 {
                return false
...
//            else {
//                if velocity.x > 0 && self.scrollView.contentOffset.x == 0 && UserConfigModel.shared.browerIndex == 0{
//                    return true
//                }
//            }
            if velocity.x < 0 {
