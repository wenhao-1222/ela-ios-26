...
public class Croper: UIView {
    ...
    public var imageWHRatio: CGFloat
    ...
    public func updateImgView(image:UIImage){
        self.image = image
//        self.imageWHRatio = image.size.width / image.size.height
        self.imageView.image = self.image
        self.imageView.contentMode = .scaleAspectFit
        self.cropWHRatio = image.size.width/image.size.height
        scrollView.zoomScale = 1
        scrollView.contentOffset = .zero
        self.recover(animated: true)
//        self.setNeedsDisplay()
//        self.imageView.center = CGPointMake(self.cropFrame.midX, self.cropFrame.midY)
//        self.imageView.center = CGPointMake(self.scrollView.frame.size.width*0.5, self.scrollView.frame.size.height*0.5)
    }
