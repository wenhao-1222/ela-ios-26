//
//  DietPlanCreateDietHistoryVM.swift
//  lns
//
//  Created by Codex on 2026/2/26.
//

class DietPlanCreateDietHistoryVM: UIView {
...
extension DietPlanCreateDietHistoryVM {
    func initUI() {
        ...
        for (index, text) in dataArray.enumerated() {
            let card = UIView()
            card.backgroundColor = WHColor_16(colorStr: "E8E8EA")
            ...
            lab.font = .systemFont(ofSize: 24/2.0, weight: .medium)
            lab.textAlignment = .center
    ...
    func select(index: Int) {
        guard index >= 0 && index < dataArray.count else { return }
