...
class JournalReportWeekMsgVM: UIView {
    ...
    var msgRequestBlock:((NSDictionary)->())?
    var snapBlock:(()->())?
...
//        let vi = ForumCommentListTableView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDHT, height: selfHeight-bottomWidthHeight), style: .grouped)
        let vi = ForumCommentListTableView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDHT, height: selfHeight), style: .grouped)
...
extension JournalReportWeekMsgVM{
...
extension JournalReportWeekMsgVM:UITableViewDelegate,UITableViewDataSource{
...
//extension JournalReportWeekMsgVM:UIScrollViewDelegate{
...
//        tableView.addSubview(rankingButton)
//        addSubview(bottomWhiteView)
//        bottomWhiteView.addSubview(shareButton)
        addSubview(nodataVm)
...
extension JournalReportWeekMsgVM{
    func sendWeeklyReposrtRequest() {
        WHNetworkUtil.shareManager().POST(urlString: URL_weekly_nutrition_report, parameters: nil,isNeedToast: true,vc: self.controller) { responseObject in
...
//                self.rankingButton.isHidden = true
                ...
                self.weekMsgDict = dataObj
                self.msgRequestBlock?(self.weekMsgDict)
                self.addSubview(self.bottomWhiteView)
                self.bottomWhiteView.addSubview(self.shareButton)
                self.tableView.addSubview(self.rankingButton)
                
                self.tableView.frame = CGRect.init(x: 0, y: 0, width: SCREEN_WIDHT, height: self.selfHeight-self.bottomWidthHeight)
                self.tableView.hideSkeleton()
                self.tableView.reloadData()
                
                self.bottomWhiteView.isHidden = false
            }
        }
    }
}
