//
//  MealAdviceNextSkeletonVC.swift
//  lns
//
...
/// 下餐规划结果加载页。
final class MealAdviceNextSkeletonVC: WHBaseViewVC {

    /// 页面对应的规划数据管理器。
    private var viewModel: MealAdviceNextViewModel?
    /// 从食物选择页带入的食物。
    private let selectedFoods: NSArray
    /// 页面日志日期。
    private let sDate: String
    /// 当前日志页点击的餐序号，1 开始，0 表示未指定。
    private let mealIndex: Int
    /// 骨架态食物行。
    private var loadingFoodItems: [MealAdviceNextLoadingFoodItem] = []
    /// 下餐规划请求的版本号，用来忽略过期回包。
    private var mealPlanRequestVersion = 0
    /// 当前是否正在请求下餐规划接口。
    private var isRequestingMealPlan = false
    /// 接口已返回但等待加载文案流程完成的结果。
    private var pendingViewModel: MealAdviceNextViewModel?
    /// 下餐规划失败 toast 显示时间。
    ...
    /// 加载中提示文案，前五段按固定顺序展示，最后一段停留到真实内容切换。
    private let loadingTipTexts = [
        "正在分析今日已摄入营养...",
        "正在计算今日剩余营养目标...",
        "正在分析所选食物的营养构成...",
        "正在匹配更合适的食物份量...",
    ...
    /// 加载文案轮播定时器。
    private var loadingTipTimer: Timer?
    /// 第六段文案显示后的延迟切换任务。
    private var loadingTipFinishWorkItem: DispatchWorkItem?
    /// 前五段文案是否已经按顺序展示完。
    private var loadingTipSequenceFinished = false
    /// 每段加载文案的展示间隔。
    ...
    /// 创建下餐规划结果加载页。
    /// - Parameters:
    ///   - selectedFoods: 用户已选择的食物。
    ///   - sDate: 从日志页进入时的日期。
    ///   - mealIndex: 从日志页进入时点击的餐序号。
    init(selectedFoods: NSArray, sDate: String, mealIndex: Int = 0) {
        self.selectedFoods = selectedFoods
        self.sDate = sDate
        self.mealIndex = mealIndex
        self.loadingFoodItems = MealAdviceNextSkeletonVC.buildLoadingFoodItems(from: selectedFoods)
    ...
    deinit {
        loadingTipTimer?.invalidate()
        loadingTipFinishWorkItem?.cancel()
...
extension MealAdviceNextSkeletonVC {
    /// 搭建页面结构。
...
//
    ...
    private func reloadData() {
        ...
        if viewModel == nil {
            configureTopMetricLoadingState()
            configureRemainingMetricLoadingState()
        } else {
    ...
    private func reloadFoodListHeight() {
        let rowCount = viewModel?.foodItems.count ?? loadingFoodItems.count
        let contentHeight = max(foodRowHeight * CGFloat(rowCount), kFitWidth(1))
    ...
    private func updateAddButtonState() {
        guard let viewModel = viewModel else {
            addToLogsButton.isEnabled = false
            addToLogsButton.alpha = 0.5
            return
        }
    ...
    private func refreshVisibleFoodCell(at index: Int) {
        guard let viewModel = viewModel, viewModel.foodItems.indices.contains(index) else { return }
        let indexPath = IndexPath(row: index, section: 0)
        guard let cell = foodTableView.cellForRow(at: indexPath) as? MealAdviceNextFoodCell else { return }
        cell.sync(with: viewModel.foodItems[index],
                  keepQuantityText: true)
    }
    ...
    private func showLoadingState() {
        ...
        configureRemainingMetricLoadingState()
        updateAddButtonState()
        startLoadingTipsRotation()
    }

    /// 配置顶部营养摘要的加载态，只将数值替换为骨架。
    private func configureTopMetricLoadingState() {
        let metrics = [
            ("热量", "千卡"),
            ("碳水", "g"),
            ("蛋白质", "g"),
        ...
        for (index, metric) in metrics.enumerated() {
            guard index < topMetricViews.count, index < metricColors.count else { continue }
            topMetricViews[index].setLoading(title: metric.0,
                                              unit: metric.1,
                                              color: metricColors[index])
        }
    }

    /// 配置本餐后剩余的加载态标题，标题直接显示在圆环下方。
    private func configureRemainingMetricLoadingState() {
        let metrics = [
            ("热量", "千卡"),
            ("碳水", "g"),
            ("蛋白质", "g"),
        ...
        for (index, metric) in metrics.enumerated() {
            guard index < ringMetricViews.count else { continue }
            ringMetricViews[index].setLoading(title: metric.0, unit: metric.1)
        }
    }
    ...
    /// 更新真实内容文案。
    private func applyLoadedContentText() {
        tipsTitleLabel.text = "建议份量不合理？"
        tipsContentLabel.text = "当所选食物无法覆盖剩余营养目标时，建议份量可能不符合预期。请根据剩余目标搭配不同食物：\n\n蛋白质：鸡胸肉、鸡腿肉、鱼肉等\n碳水：米饭、糙米、燕麦等\n脂肪：牛油果、橄榄油、坚果等"
    }

    /// 请求下餐规划接口。
    private func sendMealPlanNextRequest() {
        let restMealNum = Int(QuestinonaireMsgModel.shared.mealsPerDay) ?? 0
        let fidList = MealAdviceNextSkeletonVC.mealPlanFidList(from: selectedFoods)
        guard restMealNum > 0, fidList.count > 0 else {
            handleMealPlanNextFailure(message: "请先完成餐数和食物选择", requestVersion: mealPlanRequestVersion)
            return
        }
    ...
    /// 处理下餐规划接口成功。
    private func handleMealPlanNextSuccess(planDict: NSDictionary, requestVersion: Int) {
        guard requestVersion == mealPlanRequestVersion else { return }
        let nextViewModel = MealAdviceNextViewModel(responseDict: planDict, sDate: sDate)
        guard nextViewModel.foodItems.count > 0 else {
            handleMealPlanNextFailure(message: "系统繁忙，请稍后再试", requestVersion: requestVersion)
            return
        }
        pendingViewModel = nextViewModel
        revealLoadedContentIfReady()
    }

    /// 处理下餐规划接口失败。
    private func handleMealPlanNextFailure(message: String, requestVersion: Int) {
        guard requestVersion == mealPlanRequestVersion else { return }
        cancelMealPlanRequest()
    ...
    /// 取消当前请求。
    private func cancelMealPlanRequest() {
        mealPlanRequestVersion += 1
        isRequestingMealPlan = false
        stopLoadingTipsRotation()
    ...
    /// 启动固定顺序的加载文案流程。
    private func startLoadingTipsRotation() {
        stopLoadingTipsRotation()
        loadingTipIndex = 0
        loadingTipSequenceFinished = false
        tipsContentLabel.text = loadingTipTexts.first
        guard loadingTipTexts.count > 1 else {
            loadingTipSequenceFinished = true
            revealLoadedContentIfReady()
            return
        }
        let timer = Timer(timeInterval: loadingTipInterval, repeats: true) { [weak self] _ in
            self?.advanceLoadingTip()
        }
        RunLoop.main.add(timer, forMode: .common)
        loadingTipTimer = timer
    }

    /// 停止加载文案轮播。
    private func stopLoadingTipsRotation() {
        loadingTipTimer?.invalidate()
        loadingTipTimer = nil
        loadingTipFinishWorkItem?.cancel()
        loadingTipFinishWorkItem = nil
    }

    /// 切换下一条加载文案，达到最后一段后不再循环。
    private func advanceLoadingTip() {
        guard viewModel == nil,
              loadingTipTexts.count > 0,
              loadingTipIndex < loadingTipTexts.count - 1 else {
            stopLoadingTipsRotation()
            return
        }
        loadingTipIndex += 1
        let text = loadingTipTexts[loadingTipIndex]
        UIView.transition(with: tipsContentLabel, duration: 0.25, options: .transitionCrossDissolve) {
            self.tipsContentLabel.text = text
        }
        guard loadingTipIndex == loadingTipTexts.count - 1 else { return }

        loadingTipSequenceFinished = true
        stopLoadingTipsRotation()
        let workItem = DispatchWorkItem { [weak self] in
            self?.revealLoadedContentIfReady()
        }
        loadingTipFinishWorkItem = workItem
        DispatchQueue.main.asyncAfter(deadline: .now() + finalLoadingTipDisplayDuration,
                                      execute: workItem)
    }

    /// 等待文案流程和接口结果都完成后切换真实内容。
    private func revealLoadedContentIfReady() {
        guard loadingTipSequenceFinished,
              viewModel == nil,
              let pendingViewModel = pendingViewModel else {
            return
        }
        ...
        finishLoadingState()
        let loadingSnapshotView = view.snapshotView(afterScreenUpdates: false)
        if let loadingSnapshotView = loadingSnapshotView {
            loadingSnapshotView.frame = view.bounds
            loadingSnapshotView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        ...
        guard let loadingSnapshotView = loadingSnapshotView else { return }
        UIView.animate(withDuration: loadingContentTransitionDuration,
                       delay: 0,
                       options: [.beginFromCurrentState, .curveEaseOut, .allowUserInteraction]) {
            loadingSnapshotView.alpha = 0
        } completion: { _ in
            loadingSnapshotView.removeFromSuperview()
        }
    }
    ...
    /// 从选中的食物里提取 fid 列表。
    private static func mealPlanFidList(from selectedFoods: NSArray) -> [Int] {
        var fidList: [Int] = []
        for case let rawDict as NSDictionary in selectedFoods {
            let foodsDict = normalizedFoodDict(from: rawDict)
            let fidString = foodsDict.stringValueForKey(key: "fid")
    ...
    /// 构造骨架态食物行。
    private static func buildLoadingFoodItems(from selectedFoods: NSArray) -> [MealAdviceNextLoadingFoodItem] {
        Array(repeating: MealAdviceNextLoadingFoodItem(), count: selectedFoods.count)
    }

    /// 将原始数据规范化为食物字典。
    private static func normalizedFoodDict(from rawDict: NSDictionary) -> NSDictionary {
...
extension MealAdviceNextSkeletonVC: UITableViewDelegate, UITableViewDataSource {
    /// 返回食物列表行数。
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        viewModel?.foodItems.count ?? loadingFoodItems.count
    }

    /// 创建食物列表行。
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MealAdviceNextFoodCell") as? MealAdviceNextFoodCell
        if let viewModel = viewModel, viewModel.foodItems.indices.contains(indexPath.row) {
        ...
        } else if loadingFoodItems.indices.contains(indexPath.row) {
            cell?.onSelectionTap = nil
            cell?.onQuantityChanged = nil
            cell?.onQuantityEditingEnded = nil
            cell?.setLoading()
        }
