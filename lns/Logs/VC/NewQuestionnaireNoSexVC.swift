//
//  NewQuestionnaireNoSexVC.swift
//  lns
//
//  Created by LNS2 on 2024/6/19.
//  问卷，不需要选择性别  出生年月
...
class NewQuestionnaireNoSexVC: WHBaseViewVC {
...
//    }
    ...
    lazy var goalVm : QuestionnaireGoalVM = {
        let vm = QuestionnaireGoalVM.init(frame: CGRect.init(x: 0, y: self.progressVm.frame.maxY, width: 0, height: 0))
        vm.nextBlock = {()in
            self.step = 4
...
extension NewQuestionnaireNoSexVC{
...
extension NewQuestionnaireNoSexVC{
    func sendSurveySaveRequest() {
        ...
        if QuestinonaireMsgModel.shared.surveytype == "full"{
            ...
            DLLog(message: "sendSurveySaveRequest:\(param)")
