...
class GoalSetCircleDaysVM: UIView {
    
    let selfHeight = kFitWidth(79)
    let daysGap = (SCREEN_WIDHT-kFitWidth(32))/5
...
//                touchWhiteViewCenterX = (daysGap+circelWidth*0.5)*CGFloat(3)
    ...
    func updateProgress() {
        UIView.animate(withDuration: 0.2, delay: 0,options: .curveLinear) {
            if self.daysNumber < 6 {
                self.touchWhiteView.center = CGPoint.init(x: self.daysGap*CGFloat(self.daysNumber-1), y: kFitWidth(12))
                self.progressView.frame = CGRect.init(x: kFitWidth(2), y: kFitWidth(2), width: self.daysGap*CGFloat(self.daysNumber-1)+kFitWidth(7), height: kFitWidth(20))
                self.touchWhiteViewCenterX = self.daysGap*CGFloat(self.daysNumber-1)
            }else{
    ...
    func updateDayNumberLabel() {
        var stringOne = NSMutableAttributedString.init(string: "循环周期 ")
