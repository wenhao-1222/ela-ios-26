//
//  HabitWeeklyRewardPointAlertVM.swift
//  lns
//
...
class HabitWeeklyRewardPointAlertVM: UIView {
...
//        vi.layer.cornerRadius = whiteViewTopRadius
    ...
    lazy var subTitleLab: UILabel = {
        ...
        let attr = NSMutableAttributedString(string: "结算时间：", attributes: [.font:UIFont.systemFont(ofSize: 14, weight: .medium)])
        attr.append(NSAttributedString(string: "每周一 0 点", attributes: [.font:UIFont.systemFont(ofSize: 14, weight: .regular)]))
    ...
    lazy var dottedLineView: DottedLineView = {
        let vi = DottedLineView.init(frame: CGRect.init(x: kFitWidth(32), y: kFitWidth(127), width: SCREEN_WIDHT-kFitWidth(64), height: kFitWidth(1)))
...
// MARK: - Public API
extension HabitWeeklyRewardPointAlertVM {
    func showSelf() {
        isHidden = false
...
extension HabitWeeklyRewardPointAlertVM{
    func udpateUI(dataArray:NSArray) {
        for i in 0..<dataArray.count{
            let dict = dataArray[i]as? NSDictionary ?? [:]
            let vm = HabitWeeklyRewardPointAlertItemVM.init(frame: CGRect.init(x: 0, y: kFitWidth(8)+kFitWidth(72)*CGFloat(i), width: 0, height: 0))
            vm.updateDict(dict: dict)
            scrollView.addSubview(vm)
            scrollView.contentSize = CGSize.init(width: 0, height: vm.frame.maxY + kFitWidth(10) + WHUtils().getBottomSafeAreaHeight())
        }
    }
}
...
extension HabitWeeklyRewardPointAlertVM{
    ...
    func setConstrait() {
        ...
        scrollView.snp.makeConstraints { make in
            ...
            make.top.equalTo(kFitWidth(140))
...
extension HabitWeeklyRewardPointAlertVM{
    func sendDataRequest() {
        WHNetworkUtil.shareManager().POST(urlString: URL_user_habit_weeklyRewardPointDescription, parameters: nil) { responseObject in
            let dataString = AESEncyptUtil.aesDecrypt(hexString: responseObject["data"]as? String ?? "")
            let dataArray = WHUtils.getArrayFromJSONString(jsonString: dataString ?? "")
            DLLog(message: "sendDataRequest:\(dataArray)")
            
            self.udpateUI(dataArray: dataArray)
        }
    }
}
