...
class ServiceContactMarketVC: WHBaseViewVC {
...
//        sendOssStsRequest()
    ...
    lazy var bottomView: UIView = {
        ...
        vi.backgroundColor = .COLOR_BG_BLACK_04//.COLOR_BG_WHITE//WHColorWithAlpha(colorStr: "000000", alpha: 0.04)
...
//            }
    ...
    func checkNetWork(){
        ...
        reachability.whenReachable = { reachability in
            ...
            self.naviTitleLabel.text = "商品咨询"
        }
        reachability.whenUnreachable = { _ in
            print("Not reachable")
            let attr = NSMutableAttributedString(string: "商品咨询")
...
extension ServiceContactMarketVC:UITableViewDelegate,UITableViewDataSource{
...
extension ServiceContactMarketVC: UIScrollViewDelegate {
...
extension ServiceContactMarketVC: PHPickerViewControllerDelegate {
...
extension ServiceContactMarketVC:RITLPhotosViewControllerDelegate{
...
extension ServiceContactMarketVC {
    ...
    private func createVideoMessagePlaceholder(messageId: String, asset: AVAsset) -> NSMutableDictionary {
        ...
        messageDict.setValue(ServiceContactVC.VideoUploadState.preparing.rawValue, forKey: "videoUploadState")
    ...
    private func updateVideoMessage(messageId: String,
                                    state: ServiceContactVC.VideoUploadState? = nil,
            ...
            DispatchQueue.main.async {
                if let indexPath = self.indexPath(forMessageId: messageId),
                   let cell = self.tableView.cellForRow(at: indexPath) as? ServiceContactTableViewVideoCell {
                    ...
                    let stateStr = dict?.stringValueForKey(key: "videoUploadState") ?? ServiceContactVC.VideoUploadState.uploading.rawValue
                    let sss = ServiceContactVC.VideoUploadState(rawValue: stateStr) ?? .uploading
                    cell.updateProgressUI(state: sss, progress: CGFloat(p))
                }
            }
...
extension ServiceContactMarketVC: ServiceContactTableViewVideoCellDelegate {
    func serviceContactVideoCellDidTogglePause(_ cell: ServiceContactTableViewVideoCell) {
        ...
        guard let dict = messageDictionary(for: messageId),
              let state = ServiceContactVC.VideoUploadState(rawValue: dict.stringValueForKey(key: "videoUploadState")) else { return }
        switch state {
        case .failed, .paused:
