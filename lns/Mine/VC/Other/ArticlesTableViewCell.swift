...
extension ArticlesTableViewCell {
    override func prepareForReuse() {
        ...
        // ✅ 复用前清理：计时器 + 骨架 + 透明度/隐藏状态
        countdownTimer?.invalidate()
        countdownTimer = nil
        removeSkeletonImmediately()                    // 立即移除任何残留的骨架
        contentView.alpha = 1
...
//            titleLab.showSkeleton(cfg)
            ...
            orderIdLabel.text = "订单号 \(dict.stringValueForKey(key: "orderId"))"
            ...
            // 3) 最后统一把骨架优雅淡出 + 内容淡入
            [orderIdLabel, imgView, titleLab, subTitleLab].forEach { $0.hideSkeletonWithCrossfade() }
