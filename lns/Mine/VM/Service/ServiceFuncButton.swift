...
/// MARK: - 统一封装的可点击分享按钮
class ServiceFuncButton : UIButton {
    ...
    lazy var imgView : UIImageView = {
        let img = UIImageView(frame: CGRect(x: kFitWidth(17), y: kFitWidth(17), width: kFitWidth(30), height: kFitWidth(30)))
        img.isUserInteractionEnabled = false // [修改] 子视图不参与交互，避免抢事件
        return img
    }()
    lazy var contenLab : UILabel = {
        let lab = UILabel(frame: CGRect(x: kFitWidth(0), y: kFitWidth(69), width: kFitWidth(65), height: kFitWidth(20)))
    ...
    private func setupButton() {
        ...
        coverVi.snp.makeConstraints { make in
            make.left.top.width.height.equalTo(imgBottomView)
        }
