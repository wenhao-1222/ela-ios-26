...
-(void)updateChoiceStatus:(BOOL)isVideo{
    if ([RITLPhotosDataManager sharedInstance].count > 0 && [RITLPhotosDataManager sharedInstance].assets.count > 0) {
        PHAsset *firstAsset = [RITLPhotosDataManager sharedInstance].assets.firstObject;
        if (firstAsset.mediaType == PHAssetMediaTypeVideo){
            if (![[RITLPhotosDataManager sharedInstance].assetIdentiers containsObject:self.asset.localIdentifier]){
//                self.shadeView.hidden = NO;
//                [self setUserInteractionEnabled:NO];
                [self setChoiceStatus:NO];
            }else{
//                self.shadeView.hidden = YES;
//                [self setUserInteractionEnabled:YES];
                [self setChoiceStatus:YES];
            }
        }else{
            if (isVideo) {
//                self.shadeView.hidden = NO;
...
//    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"canChoiceVideo"]  isEqual: @"0"] && isVideo){
//        self.shadeView.hidden = NO;
//        [self setUserInteractionEnabled:NO];
//    }else{
//        self.shadeView.hidden = YES;
...
-(void)setChoiceStatus:(BOOL)canChoice{
    self.shadeView.hidden = canChoice;
    [self setUserInteractionEnabled:canChoice];
}
