...
- (PHAssetCollection *)assetCollection
{
    if (!_assetCollection) {
        ...
        _assetCollection = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeAny options:fetchOptions].firstObject;
    }
...
- (void)viewDidLoad {
    ...
    if (self.localIdentifier) {
        //加载
        self.assetCollection = [PHAssetCollection fetchAssetCollectionsWithLocalIdentifiers:@[self.localIdentifier] options:[[PHFetchOptions alloc]init]].firstObject;
    }
    ...
    [PHPhotoLibrary authorizationStatusAllow:^{
        ...
        if (RITLPhotosConfiguration.defaultConfiguration.onlyVideo) {
            option.predicate = [NSPredicate predicateWithFormat:@"mediaType == %d", PHAssetMediaTypeVideo];
        }else if (!RITLPhotosConfiguration.defaultConfiguration.containVideo) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.imageManager requestImageForAsset:asset targetSize:size contentMode:PHImageContentModeAspectFill options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
        ...
        if ([cell.representedAssetIdentifier isEqualToString:asset.localIdentifier] && result) {
            ...
            [cell updateChoiceStatus:(asset.mediaType == PHAssetMediaTypeVideo)];
            //限制超过15分钟的视频，不能选择
            NSLog(@"视频时长判断：%f  ---  max:%ld",asset.duration,RITLPhotosConfiguration.defaultConfiguration.videoMaxSeconds);
            if ((asset.mediaType == PHAssetMediaTypeVideo) && asset.duration > RITLPhotosConfiguration.defaultConfiguration.videoMaxSeconds){
                [cell setChoiceStatus:NO];
            }
        }
    }];
...
- (void)pushHorAllBrowseViewControllerWithAsset:(PHAsset *)asset
{
    ...
    browerController.modalPresentationStyle = UIModalPresentationFullScreen;
    browerController.dataSource = dataSource;
    browerController.backHandler = ^{
        [self.collectionView reloadData];
    };
    [self presentViewController:browerController animated:YES completion:nil];
//    [self presentModalViewController:browerController animated:YES];
...
//        [[NSUserDefaults standardUserDefaults]setValue:@"0" forKey:@"canChoiceVideo"];
//    }
//    canChoiceVideo
...
//        [[NSUserDefaults standardUserDefaults]setValue:@"0" forKey:@"canChoiceVideo"];
//    }else{
//        [[NSUserDefaults standardUserDefaults]setValue:@"1" forKey:@"canChoiceVideo"];
//    }
