...
@property (nonatomic,assign) Boolean isOnlyLoading;
@end
...
{
    MJRefreshCheckState
    if (self.isOnlyLoading){
        [self setStateForOnlyLoading:state and:oldState];
        return;
    }
    // 根据状态做事情
    if (state == MJRefreshStateIdle) {
        if (oldState == MJRefreshStateRefreshing) {
            ...
            [UIView animateWithDuration:self.slowAnimationDuration animations:^{
                ...
                self.arrowView.hidden = self.isOnlyLoading ? YES : NO;
            }];
        } else {
            [self.loadingView stopAnimating];
            self.arrowView.hidden = self.isOnlyLoading ? YES : NO;
...
- (void)setStateForOnlyLoading:(MJRefreshState)state and:(MJRefreshState)oldState{
    // 根据状态做事情
    self.arrowView.hidden = YES;
...
-(void)setOnlyLoadingView{
    self.isOnlyLoading = true;
}
@end
